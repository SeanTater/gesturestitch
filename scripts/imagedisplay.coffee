...
#            'images/building_horizontal.jpg'
    ...
    match: (state)->
        ...
        switch state? and state.n or 0
            ...
            when 4
                state.first.overlay(state.second, state.transform)
                $("#status").text("#{state.matches.length} matches found, centered on #{state.transform.getTranslation().x}, #{state.transform.getTranslation().y}")
        state.n++
